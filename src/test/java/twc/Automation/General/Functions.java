...
public class Functions extends TwcAndroidBaseTest{
	...
	//static CharSequence[][] exceldata = null;
	   ...
	   public static String currentday1=null;
		...
		public static boolean interStitialDisplayed = true;
...
public static void clickOnElement(By byLocatorStrategy, MobileElement element, String elementName) {
	...
	while (!element.isEnabled()) {
		...
		if (elementCheckTimeconvert >= 240) {
			...
			logStep("Noticed that " + elementName + " not enabled to perform click action, waited for: "+elementCheckTimeconvert +" Seconds, test may fail");
			break;
		} 
		try {
			Thread.sleep(2000L);
...
logStep("Clicking on " + elementName + " Element");
	element.click();
	// Driver.attachScreen();
...
waitForMilliSeconds(2000);
		interstitalAd = (MobileElement) Ad.findElementByAccessibilityId("CLOSE");
		...
		try {
			waitForMilliSeconds(2000);
			interstitalAd = (MobileElement) Ad.findElementByAccessibilityId("NO THANKS");
			interStitialDisplayed = true;
			attachScreen();
			boolean isInterstitialClosed = false;
			for (int i = 0; i <= 10; i++) {
		   ...
		   waitForMilliSeconds(2000);
				...
				try {
		waitForMilliSeconds(2000);
					interstitalAd = (MobileElement) Ad.findElementByAccessibilityId("Close Advertisement");
					/*
